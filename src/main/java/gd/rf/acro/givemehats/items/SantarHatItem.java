...
public class SantarHatItem extends Item implements Trinket {
    ...
    public void tick(PlayerEntity player, ItemStack stack) {
        if(player.getEntityWorld().getBlockState(player.getBlockPos().down()).getBlock()== Blocks.WATER)
        {
            player.getEntityWorld().setBlockState(player.getBlockPos().down(),Blocks.FROSTED_ICE.getDefaultState());
        }
    }
    ...
    public void render(String slot, MatrixStack matrixStack, VertexConsumerProvider vertexConsumer, int light, PlayerEntityModel<AbstractClientPlayerEntity> model, AbstractClientPlayerEntity player, float headYaw, float headPitch) {
        ...
        ItemStack stack = new ItemStack(GiveMeHats.SANTAR_HAT_ITEM);
