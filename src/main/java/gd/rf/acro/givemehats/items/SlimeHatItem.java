...
public class SlimeHatItem extends TrinketItem {

   @Override
    public void appendHoverText(ItemStack p_41421_, @Nullable Level p_41422_, List<Component> p_41423_, TooltipFlag p_41424_) {
        super.appendHoverText(p_41421_, p_41422_, p_41423_, p_41424_);
        p_41423_.add(new TranslatableComponent("text.slimehat"));
    }
    ...
    public void curioTick(SlotContext slotContext, ItemStack stack) {
        ...
        if(!slotContext.entity().getLevel().getBlockState(slotContext.entity().getOnPos().below()).isAir() && slotContext.entity().fallDistance>2)
        {
            Vec3 vec3 = slotContext.entity().getDeltaMovement();
            if(vec3.y < 0)
            {
                slotContext.entity().setDeltaMovement(vec3.x, -vec3.y * 0.8, vec3.z);
                slotContext.entity().fallDistance=0;
            }
        }
    }
