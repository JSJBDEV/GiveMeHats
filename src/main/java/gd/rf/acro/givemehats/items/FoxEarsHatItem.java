...
public class FoxEarsHatItem extends Item implements Trinket {


    public FoxEarsHatItem(Settings settings) {
        super(settings);
    ...
    public void appendTooltip(ItemStack stack, World world, List<Text> tooltip, TooltipContext context) {
        ...
        tooltip.add(new TranslatableText("text.foxears"));
    }
    ...
    public void tick(PlayerEntity player, ItemStack stack) {
        ...
        {
            List<AnimalEntity> entities = player.getEntityWorld().getEntitiesByClass(AnimalEntity.class,new Box(player.getX()-15,player.getY()-15,player.getZ(),player.getX()+15,player.getY()+15,player.getZ()+15), LivingEntity::isAlive);
    ...
    public void render(String slot, MatrixStack matrixStack, VertexConsumerProvider vertexConsumer, int light, PlayerEntityModel<AbstractClientPlayerEntity> model, AbstractClientPlayerEntity player, float headYaw, float headPitch) {
        ...
        ItemStack stack = new ItemStack(GiveMeHats.FOX_EARS_HAT_ITEM);
